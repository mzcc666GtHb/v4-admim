...
export default {
  ...
  methods: {
    ...
    toggleExpanded(node) {
      let nextState,siblings
      if(node.parentNode && node.parentNode.children && node.parentNode.children.length) {
        siblings = node.parentNode.children.filter(item=>{
      ...
      if (this.localSearch.active) {
        ...
        //兄弟节关闭展开
        siblings && siblings.forEach(item =>{
